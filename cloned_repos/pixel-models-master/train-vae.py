...
from torch.nn.functional import binary_cross_entropy, relu, nll_loss, cross_entropy, softmax, mse_loss
        ...
        for i in range(h):
            for j in range(w):
                for channel in range(c):
                    probs = softmax(results[:, :, channel, i, j], dim=1)
    ...
    for i in tqdm.trange(h):
        for j in range(w):
            ...
            for channel in range(c):
                ...
                probs = softmax(result[:, :, channel, i, j], dim=1)
...
def go(arg):
    ...
    if arg.task == 'mnist':
        ...
        trainloader, testloader = getloaders(arg.data_dir, batch_size=arg.batch_size)
    ...
    for epoch in range(arg.epochs):
        ...
        for i, (input, _) in enumerate(itertools.islice(tqdm.tqdm(trainloader), 451)):
            ...
            rec_loss = cross_entropy(rec, target, reduction='none').view(b, -1).sum(dim=1)
            rec_loss = rec_loss * util.LOG2E  # Convert from nats to bits

            loss = (rec_loss + kl_loss).mean()

            instances_seen += input.size(0)
            tbw.add_scalar('pixel-models/vae/training/kl-loss',  kl_loss.mean().item(), instances_seen)
            tbw.add_scalar('pixel-models/vae/training/rec-loss', rec_loss.mean().item(), instances_seen)
        ...
        if epoch % arg.eval_every == 0 and epoch != 0:
            ...
            with torch.no_grad():
                ...
                if not arg.skip_test:
                    for i, (input, _) in enumerate(itertools.islice(tqdm.tqdm(testloader), 21)):
                        if arg.limit is not None and i * arg.batch_size > arg.limit:
                            break
                        ...
                        recons = draw_pixel_sample(rec)
                        recon_loss = mse_loss(recons, input, reduction='sum') / input.shape[0]
                        mse.append(recon_loss)

                    tbw.add_scalar('pixel-models/test-loss', sum(err_te)/len(err_te), epoch)
                    print('epoch={:02}; mse: {:.3f}; training loss: {:.3f}; test loss: {:.3f}'.format(
                        epoch, sum(mse)/len(mse), sum(err_tr)/len(err_tr), sum(err_te)/len(err_te)))
                ...
                utils.save_image(recons, 'recons_celeb_{:02d}.png'.format(epoch), nrow=12, padding=0)
                utils.save_image(sample, 'sample_celeb_{:02d}.png'.format(epoch), nrow=12, padding=0)
